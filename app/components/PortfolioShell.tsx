type ProjectCardProps = { number: string; title: string; english: string; subtitle: string; href: string; image: string; srcSet?: string };
...
export function ProjectCard({ number, title, english, subtitle, href, image, srcSet }: ProjectCardProps) {
  return <a className="portfolio-project-card" href={href}><figure><img src={image} srcSet={srcSet} sizes="(max-width: 800px) 100vw, 50vw" alt="" loading="lazy" decoding="async" /></figure><div className="portfolio-project-meta"><span className="portfolio-project-number">{number}</span><div><p>{subtitle}</p><h3>{title}</h3><h4>{english}</h4></div><span className="portfolio-arrow" aria-hidden="true">↗</span></div></a>;
}
